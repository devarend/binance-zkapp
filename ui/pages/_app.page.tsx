...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      if (!state.hasBeenSetup) {
        ...
        const publicKeyBalance = PublicKey.fromBase58(
          "B62qr74C34W4PDKMXAtqSpqgx9k9bH5mxnn5m4ztbWn9Cs8hbaNjs9B"
        );
        const publicKeyTrader = PublicKey.fromBase58(
          "B62qrhTLkX6XxFE4X7epBrF3HyZKVpXwgA7bhjt4YfsUYgqWtzVKSCo"
        );
        ...
        const verifiedBalanceNum =
          await zkappWorkerClient.getVerifiedBalanceNum();
        const verifiedTraderNum =
          await zkappWorkerClient.getVerifiedTraderNum();
  ...
  const refreshState = async () => {
    ...
    await state.zkappWorkerClient!.fetchAccount({
      publicKey: publicKeyBalance,
    });
    await state.zkappWorkerClient!.fetchAccount({ publicKey: publicKeyTrader });

    const verifiedBalanceNum =
      await state.zkappWorkerClient!.getVerifiedBalanceNum();
    const verifiedTraderNum =
      await state.zkappWorkerClient!.getVerifiedTraderNum();
  ...
  const checkContract = async (contract: "Balance" | "Trader") => {
    ...
    try {
      const response = await axios.get(
        `https://binance-oracle-3mgae.ondigitalocean.app/${endpoint}`
      );
  ...
  const createTrades = async () => {
    ...
    try {
      await axios.post(
        "https://binance-oracle-3mgae.ondigitalocean.app/createTrades"
      );
    } catch (error) {}
    ...
    <div className="hero min-h-screen">
      <div className="bg-primary rounded-lg hero-content text-center">
        <div className="max-w-md">
          ...
          <p className="mb-5">
            Mina oracle which connects to the Binance Test API
          </p>
          <div className="h-14 mb-2">
            ...
            {isReady && (
              <button onClick={refreshState} className="btn btn-outline mt-1">
                ↻ Refresh state
              </button>
            )}
            ...
            {walletNotFound && (
              <div className="bg-black">
                <h1 className="text-white p-2">
                  Could not find a wallet. Install{" "}
                  <a
          ...
          <div className="flex flex-col">
            <Button
              onClick={() => {
                setModal({
                  ...
                  description:
                    "Connect to the Binance oracle and make sure that the BNB balance is at least 1000 or more.",
            ...
            >
              Balance check
              {isReady &&
                `(verified amount: ${state.verifiedBalanceNum!.toString()})`}
            </Button>
            ...
            <Button
              onClick={() => {
                setModal({
                  ...
                  description:
                    "Connect to the Binance oracle and make sure that you have 10 trades or more.",
                  buttonText: "Create proof",
            ...
            >
              Trader check
              {isReady &&
                `(verified amount: ${state.verifiedTraderNum!.toString()})`}
            </Button>
            <div className="divider">OR</div>
            <Button onClick={createTrades} isLoading={isCreatingTrades}>
              Create trades
            </Button>
          </div>
