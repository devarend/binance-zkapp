...
const functions = {
  ...
  loadContract: async (args: {}) => {
    const { BalanceContract } = await import(
      "../../contracts/build/src/BalanceContract.js"
    );
    const { TraderContract } = await import(
      "../../contracts/build/src/TraderContract.js"
    );
  ...
  initZkappInstance: async (args: {
    publicKeyBalance: string;
    publicKeyTrader: string;
  }) => {
  ...
  createUpdateTransaction: async (args: {
    ...
    const transaction = await Mina.transaction(() => {
      contract === "Balance"
        ? state.balancezkapp!.verify(
            Field(id),
            Field(data),
            Signature.fromJSON(signature)
          )
        : state.traderzkapp!.verify(
            Field(id),
            Field(data),
            Signature.fromJSON(signature)
          );
