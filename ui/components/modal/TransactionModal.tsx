...
const TransactionModal: FC<TransactionModalProps> = ({ id, title, description, transactionHash, isVisible = false, close, error = false}) => {
    const openModalClassName = isVisible ? 'modal-open' : null
    const openTransaction = () => {
        window.open('https://berkeley.minaexplorer.com/transaction/' + transactionHash, '_blank');
    }
                ...
                <label className="modal-box relative" htmlFor="">
                    ...
                    <div className="my-4">
                        {error ? <>
                            ...
                            <button onClick={openTransaction} className="btn btn-sm btn-primary text-black">Open
                                transaction
                            </button>
                        </div>}
                    </div>
                    <div className="modal-action">
                        <Button onClick={close} className="btn-primary text-black">Close</Button>
                    </div>
                </label>
            </label>
        </>
